...
export async function updateLeagueRankings(eventId: number, matchId: number): Promise<boolean> {
    try {
        ...
        // その試合結果がリーグ戦のものかどうか確認
        {
                ...
                match.teamIds.every(async teamId => {
                        ...
                        if (at.type === "T") {
                            if (at.matchId === matchId) {
                                ...
                                const matchPlan = await prisma.matchPlan.findUnique({
                                    where: {id: parseInt(String(at.matchId))}
                                });
                            ...
                            if (at.eventId === eventId) {
                                return blockStatus && blockStatus[blockName] && blockStatus[blockName].completed;
                            }
                        }
            ...
            console.log(`依存している試合の数: ${dependentMatchPlansWithAllTeams.length}`);
            console.log(`依存している試合のID: ${dependentMatchPlansWithAllTeams.map(match => match.id)}`);
            console.log(`依存している試合のチームID: ${dependentMatchPlansWithAllTeams.map(match => match.teamIds)}`);
            console.log(`依存している試合のステータス: ${dependentMatchPlansWithAllTeams.map(match => match.status)}`);
        ...
        if (blockStatus.completed) { // もしこのリーグが予選で本選の試合がこのリーグに依存している場合はその試合の依存チーム情報が出揃っている場合に限ってstatusをpreparingにする
            // 予選の試合を取得
            ...
            const dependentMatchPlansWithAllTeams = dependentMatchPlans.filter(match => {
                return match.teamIds.every(async teamId => {
                    const at = AnalyzeVariableTeamId(teamId)
                    if (at === null) {
                        return true; // 変数IDではない場合は準備okとみなす
                    }
                    if (at.type === 'L') {// ブロック内のチームIDの場合
                        // そのブロックの結果が出ているか確認する
                        if (at.eventId === eventId && at.blockName === blockName) { // 今処理中のブロックのことだったならそのままok
                            return true;
                        } else {
                        ...
                        const matchPlan = await prisma.matchPlan.findUnique({
                            where: {id: parseInt(at.matchId.toString())}
                        });
                        return matchPlan && matchPlan.status === 'Completed';
                    }
                });
