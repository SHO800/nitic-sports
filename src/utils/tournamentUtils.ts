...
export const implementsTournamentMatchNode = (obj: any): obj is TournamentMatchNode => {
    return (
        obj &&
...
function constructTournament(
    ...
    relatedMatchPlans.forEach(matchPlan => {
        ...
        const matchNode: TournamentNode = {
            ...
            row: 0, // 初期値
            column: calculateRound(matchPlan, relatedMatchPlans, true, TeamInfos) + 1,
            tournamentMatchNode: {
                matchId: matchPlan.id,
                ...
                if (analyzed.type === 'T') { // あるトーナメント制の試合結果に依存する場合
                    ...
                    if (matchPlanMap.has(referencedMatchId)) { // その依存先の試合がこのトーナメント内の試合であるならば
                        ...
                        matchNode.tournamentMatchNode.premiseNode![index] = { // この試合の依存関係にその参照先の試合情報を追加
                            ...
                            type: "match",
                            row: 0, // 初期値
                            column: calculateRound(referencedMatch, relatedMatchPlans, true, TeamInfos) + 1,
                            nextNode: matchNode,
                            tournamentMatchNode: {
                        ...
                        if (analyzed.condition === "W") {
                            const referencedMatchNode = nodes.find(node => node.type === "match" && (node.matchId === referencedMatch.id));
                            if (referencedMatchNode && referencedMatchNode.type === "match") {
    ...
    if (dependencies.length === 0 || (detect3rdMatch && dependencies.every(a => a.type === "T" && a.condition === "L"))) {
        ...
        dependencies.forEach((dep: TeamIdVariableDataType) => {
            if (dep.type == "T") {
                ...
                if (referencedMatch) {
                    const round = calculateRound(referencedMatch, allMatches, detect3rdMatch, teamInfos);
