...
import type {Event, MatchPlan, MatchResult} from "@prisma/client";
    ...
    (
        ...
        if (isTimeBased) {
            ...
            relatedMatchResults.forEach((result) => {
                result.teamIds.forEach((teamId, index) => {
                    ...
                    const totalMs =
                        ...
                        minutes * 60000 +
                        seconds * 1000 +
                        (millis ? Number.parseInt(millis) : 0);
            ...
            if (relatedMatchPlans.length === 5) {
                ...
                const equallyDividedByFour = new Array(length)
                    .fill(0)
                    .map((_, i) => teamInfos.slice(i * classCount, (i + 1) * classCount));
                const result = equallyDividedByFour.map((teamsByGrade) => {
                    return teamsByGrade
                        .map((team) => {
                            return sortedTeams.find(
                                (teamWithRank) =>
                                    teamWithRank.teamId.toString() === team.teamId,
                            );
                        })
        ...
        // 勝利数の降順、同点ならteamId昇順で安定ソート
        remainingTeams.sort((a, b) => {
            const aTotalWinPoint = a.wins;
            const bTotalWinPoint = b.wins;
            if (bTotalWinPoint !== aTotalWinPoint) {
        ...
        remainingTeams.forEach((team, index) => {
            ...
            if (index === 0) {
                currentRank = 5;
                sameRankCount = 1;
            } else if (totalWins === prevTotalWins) {
                sameRankCount++;
            } else {
...
export const calcEventTotalRankings = cache(
    (
        ...
        const eventMatches = allMatchPlans.filter(
            (match) => match.eventId === event.id,
        );
        const eventMatchResults = allMatchResults.filter((result) =>
            eventMatches.some((match) => match.id === result.matchId),
        );
            ...
            if (teamData.type === "league") {
                ...
                Object.entries(leagueBlocks).forEach(([blockName, teams]) => {
                    results = [
                        ...teams.map((team) => {
                            return {
                                teamId: Number.parseInt(team.teamId),
                                rank: team.rank!,
...
export const calcEventScore = cache(
    (
        event: Event,
        allMatchPlans: MatchPlan[],
        allMatchResults: MatchResult[],
    ): RankWithEventScore[][] => {
        const ranking = calcEventTotalRankings(
            event,
            allMatchPlans,
            allMatchResults,
        );
