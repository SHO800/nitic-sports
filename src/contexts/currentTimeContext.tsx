...
export const useCurrentTimeContext = () => {
	...
	if (!context) {
		throw new Error(
			"useCurrentTimeContext must be used within a CurrentTimeContextProvider",
		);
	}
...
export const CurrentTimeContextProvider = ({
	children,
}: { children: ReactNode }) => {
	return (
		<CurrentTimeContext.Provider value={useCurrentTime()}>
			{children}
