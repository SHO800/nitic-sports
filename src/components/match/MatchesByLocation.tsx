...
import type { Event, Location, MatchPlan } from "@prisma/client";
import {
	type RefObject,
	type UIEventHandler,
	useEffect,
...
const MatchesByLocation = ({
	...
	useEffect(() => {
		if (!scrollerRef.current || refs.current.includes(scrollerRef.current))
			return;
	...
	useEffect(() => {
		...
		const relatedMatchPlans: MatchPlan[] = matchPlans.filter(
			(matchPlan) =>
				matchPlan.locationId === location.id &&
				(isShowCompletedMatch || matchPlan.status !== "Completed"),
		);
		// 開始時刻で並べ替え
		relatedMatchPlans.sort(
			(a, b) =>
	...
	return (
		...
		>
			<div
				className={
					"ml-8 pr-8 mt-5 pb-2 w-full text-4xl font-bold border-b-2 [border-image:linear-gradient(to_right,#888,#888_60%,#eee)_1]"
				}
			>
				{location.name}
			</div>
			...
			<div
				className={
					"flex flex-col items-center px-4 m-8 space-y-8 pt-4 pb-12 w-full overflow-x-scroll overscroll-none hidden-scrollbar"
				}
				onScroll={onScroll}
				ref={scrollerRef}
			>
