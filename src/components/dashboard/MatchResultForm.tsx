...
export const MatchResultForm = ({
	...
	const [actualTeamIds, setActualTeamIds] = useState<number[]>(
		matchResult ? matchResult.teamIds : [],
	);
	const [actualMatchScores, setActualMatchScores] = useState<string[]>(
		matchResult ? matchResult.matchScores : [],
	);
	const [actualWinnerTeamId, setActualWinnerTeamId] = useState<number>(
		matchResult ? matchResult.winnerTeamId : 0,
	);
	...
	useEffect(() => {
		...
		const variableTeamIds = matchPlan.teamIds.filter((id) =>
			id.startsWith("$"),
		);
		...
		if (variableTeamIds.length > 0) {
			...
			setCanInput(
				variableTeamIds.every((id) =>
					isFixedMatchResultOrBlockRankByVariableId(id),
				),
			);
		}
	...
	return (
		<div>
			<div
				className={`flex items-center ${canInput ? "" : "opacity-50 pointer-events-none"}`}
			>
				<form
					onSubmit={async (e) => {
						e.preventDefault();
												...
												<>
													<input
														...
														onChange={(e) => {
															...
															if (!newMatchScores[index])
																newMatchScores[index] = "::.";
														...
														value={
															actualMatchScores[index]
																? actualMatchScores[index].split(":")[0]
																: ""
														}
													/>
													:
													<input
														...
														value={
															actualMatchScores[index]
																? actualMatchScores[index].split(":")[1]
																: ""
														}
													/>
													:
										...
										<td>
											<input
												...
												onChange={(e) => {
													...
													const otherTeam = actualTeamIds.filter(
														(ati) => ati.toString() !== e.target.value,
													);
													if (otherTeam.length === 1)
														setActualLoserTeamId(otherTeam[0]);
												}}
