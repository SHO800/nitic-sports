...
const EventForm = () => {
	...
	const addTournamentTeamsFromPlans = (teamDataIndex: number) => {
		...
		const plans = matchPlans?.filter(
			(plan) => plan.eventId === eventId && plan.id >= Number(minPlanId.value),
		);
		if (!plans) return;
		const teamIds = plans.flatMap((plan) => plan.teamIds);
		...
		setTeamDataJsonDraft((prevState) => {
			...
			if (!teamDataWithIndex || teamDataWithIndex.type !== "tournament")
				return prevState;
				...
				<div className={"flex flex-col justify-start items-start"}>
					...
					<div>
						<input
							...
							className="border border-gray-400 px-4 py-2 mr-2 rounded text-black"
						/>
						<label htmlFor={"isTimeBased"} className="text-black mr-2">
							タイムレース制
						</label>
					</div>

					{isTwoStageCompetition && <p>予選</p>}
					<div className={"ml-4"}>
						<input
							type={"radio"}
							name={"eventType1"}
						...
						/>
						<label htmlFor={"eventType1Tournament"} className="text-black mr-2">
							トーナメント
						</label>
						<input
							type={"radio"}
						...
						/>
						<label htmlFor={"eventType1League"} className="text-black mr-2">
							リーグ戦
						</label>
						<div>
							<p>チーム入力</p>
					...
					{isTwoStageCompetition && (
						<>
							...
							<div className={"ml-4"}>
								...
								/>
								<label htmlFor={"eventType2League"} className="text-black mr-2">
									リーグ戦
								</label>
								<div>
									<p>チーム入力</p>
