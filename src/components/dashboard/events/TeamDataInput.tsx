		...
		return (
			<>
				{teamData.map((key, idx) => (
					<div
						key={`teamDataJsonDraft[${index}]TournamentDiv${idx}`}
						className={""}
					>
						<button
							onClick={() => {
						...
						/>
						<span>
							何階シード? (0: 普通, 1: 一試合スキップ, 2: 二試合スキップ...)
						</span>
					</div>
				))}
				<button
	...
	return (
		<>
			{Object.keys(teamDataJsonDraftElement.blocks).map((key, blockIndex) => (
				<div
					key={`teamDataJsonDraft[${index}]LeagueDiv${blockIndex}`}
					className={"flex flex-col"}
				>
					<p>{key}</p>
					{teamDataJsonDraftElement.blocks[key].map((_team, teamIndex) => (
						<input
			...
			<button
				...
				onClick={() => {
					...
					if ("blocks" in teamDataElem) {
						const blockName = String.fromCharCode(
							65 + Object.keys(teamDataElem.blocks).length,
						);
