...
import DXBracket from "./Deluxe/DXBracket";
    ...
    return(
        <div className="flex fixed inset-0 z-100 w-full  h-screen bg-black/30 justify-center items-center">
            ...
            <div className="flex flex-col max-h-[80vh] px-1 pb-4 bg-gray-300 rounded item-center overflow-auto">  
                <button onClick={() => closeModal()} className="flex absolute z-50 right-0 mt-1 mr-6 px-2 py-1 bg-red-400 hover:bg-red-300 rounded text-white">×</button>
                ...
                <div className="bg-gray-100 mt-1 rounded">
                    ...
                    {matchPlans && matchPlans.length > 0 && (
                        <DXBracket eventId={matchPlan.eventId} matchPlans={matchPlans} teamIds={matchPlan.teamIds} />
                    )}
                </div>
            </div>
            </div>
