		...
		>
			{match.matchId && (
				<div className="text-md text-gray-500 pr-2 absolute  h-full w-full flex justify-end items-center">
					<p
						className={`${match.tournamentMatchNode.matchPlan.teamIds === teamIds ? "bg-amber-500 text-white rounded" : ""}`}
					>
						<p
							className={`${match.tournamentMatchNode.matchPlan.teamIds === teamIds ? "animate-pulse text-3xl" : ""}`}
						>
			...
			<div className="relative h-full" ref={matchBoxRef}>
				<TournamentLine
					...
					color1={false ? "rgb(255,0,0)" : "rgba(156, 163, 175, 0.8)"}
					color2={false ? "rgb(255,0,0)" : "rgba(156, 163, 175, 0.8)"}
					thickness={4}
