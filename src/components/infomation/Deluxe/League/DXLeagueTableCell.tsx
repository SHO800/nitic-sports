...
const DXLeagueTableCell = ({
	...
	if (match.status === "Waiting") {
		return (
			<div
				key={i_key}
				className={`text-sm p-2 ${match.teamIds === teamIds ? "bg-amber-500" : ""}`}
			>
				<p
					className={`text-sm ${match.teamIds === teamIds ? "animate-pulse text-white text-[17px]" : ""}`}
				>
					({matchStr})
				</p>
			</div>
